...
CREATE TABLE IF NOT EXISTS `category` (
  ...
  `allow_comments` char(1) DEFAULT NULL COMMENT '栏目是否允许评论',
...
CREATE TABLE IF NOT EXISTS `topic` (
  ...
  `allow_user_post` char(1) DEFAULT NULL COMMENT '是否允许普通用户发帖',
  `allow_comments` char(1) DEFAULT NULL COMMENT '是否允许评论',
  `place_top` char(1) DEFAULT NULL COMMENT '是否置顶',
  `view_count` int(11) DEFAULT NULL COMMENT '查看次数',
  `comments_count` int(11) DEFAULT NULL COMMENT '评论条数',
  `praise_count` int(11) DEFAULT NULL COMMENT '评论条数',
  `create_time` datetime NOT NULL COMMENT '创建时间',
  `publish_time` datetime DEFAULT NULL COMMENT '发布时间',
  `last_replyer_id` bigint(64) DEFAULT NULL COMMENT '发布时间',
  `last_replyer` varchar(64) DEFAULT NULL COMMENT '发布时间',
